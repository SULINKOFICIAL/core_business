...
class NewsCategoryProcessingController extends Controller
{
    /**
    ...
    public function formatResults($query)
    {
        return DataTables::eloquent($query)
            ->addColumn('preview', function ($category) {
                return '<span class="badge badge-light-success" style="background: ' . hex2rgb($category->color, 15) . '; color: ' . e($category->color) . '">' . e($category->name) . '</span>';
            })
            ...
            ->addColumn('actions', function ($category) {
                return '<div class="d-flex gap-4 align-items-center justify-content-center">'
                    . '<a href="' . route('news.categories.edit', $category->id) . '" class="btn btn-sm btn-primary btn-active-success fw-bolder text-uppercase py-2">Editar</a>'
                    . '</div>';
            })
            ->rawColumns(['preview', 'status_label', 'created_at', 'actions'])
            ->make(true);
    }
}
